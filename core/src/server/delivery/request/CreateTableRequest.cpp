...
CreateTableRequest::Create(const std::shared_ptr<Context>& context, const std::string& table_name, int64_t dimension,
                           int64_t index_file_size, int64_t metric_type, const std::string& enc_type) {
    return std::shared_ptr<BaseRequest>(
        new CreateTableRequest(context, table_name, dimension, index_file_size, metric_type, enc_type));
}
...
CreateTableRequest::OnExecute() {
    ...
    try {
        ...
        table_info.enc_type_ = enc_type_;
